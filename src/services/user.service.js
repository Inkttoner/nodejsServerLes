...
const userService = {
    create: async (user, callback) => {
        logger.trace(
            `UserService: create new user with email: ${user.emailAdress}`
        );
        try {
    ...
    changeUser: async (userId, validationId, user, callback) => {
        ...
        try {
            ...
            if (!result || result.length < 1) {
                throw {
                    status: 404,
                    message: `User with id: ${validationId} not found!`,
                    data: {},
                };
            }
            ...
            await query(
                `UPDATE user SET firstName = '${user.firstName}', lastName = '${
                    user.lastName
                }', isActive = '${user.isActive}', emailAdress = '${
                    user.emailAdress
                }', password = '${user.password}', phoneNumber = '${
                    user.phoneNumber
                }', roles = '${user.roles}', street = '${
                    user.street || ""
                }', city = '${user.city || ""}' WHERE id = ${userId};`
            );
    ...
    deleteUser: async (userId, validationId, callback) => {
